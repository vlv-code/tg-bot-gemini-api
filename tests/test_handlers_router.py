...
class TestHandlersModularization(unittest.TestCase):
    def test_main_router_composition(self):
        """Проверяем, что главный роутер включает все 5 под-роутеров в правильном порядке."""
        self.assertIsNotNone(router)
        sub_routers = router.sub_routers
        self.assertEqual(len(sub_routers), 5)
        ...
        self.assertIs(sub_routers[3], chat_router)
        self.assertIs(sub_routers[4], inline_router)
